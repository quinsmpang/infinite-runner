 ...
 */
 package com.playdom.gas.anims 
 {
	...
	/**
	 * Waits for a mouse rollover.
	 * 
	 * @author Rob Harris
	 */
	public class OnRollover extends AnimBase	
	{
		/** The recycling pool */
         ...
         */
        public static function make(alist:AnimList, wait:int, dur:uint, proto:String, target:String, context:Object):OnRollover 
        {
			// recycle or create an instance
			if (pool.length > 0)
			{
				var anim:OnRollover = pool.pop();
			}
			else
			{
				anim = new OnRollover();
		...
		override public function destroy():void 
		{
			if (alist && alist.dob)
			{
				alist.dob.removeEventListener(MouseEvent.MOUSE_OVER, handleRollover);
			}
			super.destroy();
			protoName = null;
		...
		override public function animate():Boolean
		{
			...
			{	// past the wait period
				...
				{	// first time past wait period
					active = true;
					alist.dob.addEventListener(MouseEvent.MOUSE_OVER, handleRollover);
				...
				{	// display object has been clicked
					...
					{	// create a prototype
						...
						{	// no target specified; create an independant prototype
							...
							if ( alist2.dob )
							{
//								ViewUtils.move( alist2.dob, alist.dob.x, alist.dob.y );
								alist2.dob.x = alist.dob.x;
								alist2.dob.y = alist.dob.y;
							}
						}
					}
					rolled = false;
		 ...
		 */
		private function handleRollover(evt:Event):void
		{
			rolled = true;	// set flag to be used in the next animation call
			if (!loop)
